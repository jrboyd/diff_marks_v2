...
#     m = mods[a]
load_MAnorm = function(l1, l2, m, p_thresh = 0, default_pval = 50, ma_norm_dir = "data/ma_norm_results"){
  if(substr(ma_norm_dir, nchar(ma_norm_dir), nchar(ma_norm_dir)) != '/'){
    ma_norm_dir = paste0(ma_norm_dir, '/')
  }
  # columns of res tables will be chr,start,end,up/down,log10pval,...
  save_name = paste0(ma_norm_dir, paste(l1, m, "vs", l2, m, sep = "_"), ".save")
  print(paste("will save to", save_name))
  if (file.exists(save_name)) {
    print("loading precalculated results...")
    load(save_name)
    
  } else {
    regex = paste0(paste(l1, m, sep = "_"), ".+", paste(l2, m, sep = "_"), ".+result.xls")
    print(paste("searching for", regex))
    fname = dir(path = ma_norm_dir, full.names = T, pattern = paste0(paste(l1, m, sep = "_"), ".+", paste(l2, m, sep = "_"), ".+result.xls"))
...
if(doProstate){
  ...
  for(i in 1:3){
    ...
    for(hm in histone_mods){
      res = load_MAnorm(cell_lines[a], cell_lines[b], hm, ma_norm_dir = "data/prostate_data/ChIPseq/ma_norm/")
      MAnorm_res[[paste(cell_lines[a],hm, cell_lines[b], hm, sep = '_')]] = res
    }
  }
  save(MAnorm_res, file = 'data/precalc_prostate/MAnorm_res.save')
}else{
